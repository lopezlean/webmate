...
const run = require('gulp-run-command').default; // Run commands
 ...
 *
 */
gulp.task('editor', run(`yarn --cwd ${config.EDITOR_PATH} run build `,{quiet:false}));
...
gulp.task(
	...
	gulp.parallel('styles', 'vendorsJS', 'customJS', 'images', browsersync, () => {
		...
		gulp.watch(config.watchEditorSass, gulp.series('editor', reload)); // Reload on Editor Sass file changes.
		gulp.watch(config.watchEditorTypescript, gulp.series('editor', reload)); // Reload on Editor Typescript file changes.
	})
);
